...
class ExportPointController extends Controller
{
     ...
     * @return \Symfony\Component\HttpFoundation\BinaryFileResponse|\Illuminate\Http\JsonResponse
     */
    public function exportInsufficientSocialPointsByClass(Request $request)
    {
        try {
            // Validate request - không cần semester_id
            $validator = Validator::make($request->all(), [
            ...
            // Gọi service xuất file - danh sách SV thiếu điểm CTXH
            $result = $this->exportService->exportInsufficientSocialPointsByClass(
                $request->class_id
            );
            ...
            return response()->json([
                'success' => false,
                'message' => 'Lỗi khi xuất danh sách sinh viên thiếu điểm CTXH',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * Xuất danh sách sinh viên thiếu điểm CTXH theo khoa (< 170 điểm, lấy faculty_id từ admin đang đăng nhập)
     * 
     * @param Request $request
     * @return \Symfony\Component\HttpFoundation\BinaryFileResponse|\Illuminate\Http\JsonResponse
     */
    public function exportInsufficientSocialPointsByFaculty(Request $request)
    {
        try {
            // Không cần validate - lấy faculty_id từ admin đang đăng nhập
            ...
            // Gọi service xuất file - danh sách SV thiếu điểm CTXH với faculty_id từ admin
            $result = $this->exportService->exportInsufficientSocialPointsByFaculty(
                $admin->unit_id
            );
