...
-   **training_points** (integer|null): Điểm rèn luyện từ học kì gần nhất. Giá trị là `null` nếu chưa có học kì nào.
-   **training_semester** (object|null): Thông tin học kì được dùng để tính điểm rèn luyện
    -   **semester_id** (integer): ID của học kì
    -   **semester_name** (string): Tên học kì
    -   **academic_year** (string): Năm học
-   **social_points** (integer): Điểm công tác xã hội tích lũy từ đầu khóa học đến hiện tại

**Notes:**

-   Điểm rèn luyện được tính từ học kì gần nhất (based on `start_date`)
-   Điểm CTXH là điểm tích lũy từ đầu, không phụ thuộc vào học kì
-   Cả hai loại điểm đều sử dụng `PointCalculationService` để tính toán
-   Nếu có lỗi khi tính điểm, `training_points` và `training_semester` sẽ là `null`, còn `social_points` sẽ là `0`
